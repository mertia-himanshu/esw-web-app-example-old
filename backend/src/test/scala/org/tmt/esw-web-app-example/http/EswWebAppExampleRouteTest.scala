...
import org.tmt.esw-web-app-example.TestHelper
...
class EswWebAppExampleRouteTest extends AnyWordSpec with ScalatestRouteTest with AkkaHttpCompat with BeforeAndAfterEach with HttpCodecs {

  private val service1: EswWebAppExampleImpl                   = mock[EswWebAppExampleImpl]
  private val service2                               = mock[JEswWebAppExampleImplWrapper]
  ...
  private val route: Route = new EswWebAppExampleRoute(service1, service2, securityDirectives).route

  override protected def beforeEach(): Unit = reset(service1, securityDirectives)

  "EswWebAppExampleRoute" must {
  ...
  val connection: Connection.HttpConnection = HttpConnection(ComponentId(Prefix(TestHelper.randomSubsystem, "esw-web-app-example"), Service))
}
